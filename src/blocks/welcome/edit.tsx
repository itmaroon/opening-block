...
import DynamicWelcomeSvg from "./DynamicWelcomeSvg";
import {
	animateDynamicPen,
	positionDynamicPenAtStart,
} from "./dynamicPenAnimation";
import { generateWelcomePaths } from "./generateWelcomePaths";
import { normalizeCenterlineOrder } from "./centerlinePath";
...
export default function Edit({
	...
	const [visibleCenterlineIndices, setVisibleCenterlineIndices] = useState<
		number[]
	>([]);
	const [draggedCenterlineIndex, setDraggedCenterlineIndex] = useState<
		number | null
	>(null);
	const [dragOverCenterlineIndex, setDragOverCenterlineIndex] = useState<
		number | null
	>(null);
	const hasDynamicPaths =
		outline_paths.length > 0 &&
		centerline_paths.length > 0 &&
		logo_width > 0 &&
		logo_height > 0;
	const isDynamicAnimationPlaying = trigger_anime && is_anime;
	const normalizedCenterlineOrder = normalizeCenterlineOrder(
		centerline_order,
	...
	useEffect(() => {
		setVisibleCenterlineIndices(
			centerline_paths.map((_, index) => index),
		);
	}, [centerline_paths]);

	const toggleCenterlineVisibility = (index: number): void => {
		setVisibleCenterlineIndices(
			visibleCenterlineIndices.includes(index)
				? visibleCenterlineIndices.filter((item) => item !== index)
				: [...visibleCenterlineIndices, index].sort((a, b) => a - b),
		);
	};

	const reorderCenterline = (
		sourceIndex: number,
		targetIndex: number,
	): void => {
		...
		const nextOrder = [...normalizedCenterlineOrder];
		const sourcePosition = nextOrder.indexOf(sourceIndex);
		const targetPosition = nextOrder.indexOf(targetIndex);
		if (sourcePosition < 0 || targetPosition < 0) {
			return;
		}
		nextOrder.splice(sourcePosition, 1);
		nextOrder.splice(targetPosition, 0, sourceIndex);
		setAttributes({ centerline_order: nextOrder });
	};

	const handleCenterlineDragStart = (
		...
		setDraggedCenterlineIndex(sourceIndex);
	};

	const handleCenterlineDragOver = (
		...
		setDragOverCenterlineIndex(targetIndex);
	};

	const handleCenterlineDrop = (
		event: DragEvent<HTMLDivElement>,
		targetIndex: number,
	): void => {
		event.preventDefault();
		...
		const sourceIndex =
			draggedCenterlineIndex ??
			(transferredIndex === "" ? Number.NaN : Number(transferredIndex));
		if (Number.isInteger(sourceIndex)) {
			reorderCenterline(sourceIndex, targetIndex);
		}
		setDraggedCenterlineIndex(null);
		setDragOverCenterlineIndex(null);
	};

	const handleCenterlineDragEnd = (
		event: DragEvent<HTMLElement>,
	): void => {
		event.stopPropagation();
		setDraggedCenterlineIndex(null);
	...
	const toggleCenterlineDirection = (sourceIndex: number): void => {
		const nextReversed = centerline_paths.map(
			(_, index) => Boolean(centerline_reversed[index]),
		);
		nextReversed[sourceIndex] = !nextReversed[sourceIndex];
		setAttributes({ centerline_reversed: nextReversed });
	};

	useEffect(() => {
		let cancelled = false;
		const timer = window.setTimeout(() => {
			void generateWelcomePaths({
				text: welcome_text,
				fontFile: welcome_font,
				fontSize: logo_size,
				letterGap: logo_gap,
				skeletonScale: skeleton_scale,
			})
				.then((geometry) => {
					if (cancelled) {
						return;
					}
					const keepsExistingAdjustments =
						geometry.centerlinePaths.length === centerline_paths.length &&
						geometry.centerlinePaths.every(
							(path, index) => path === centerline_paths[index],
						);
					const nextOrder = keepsExistingAdjustments
						? normalizeCenterlineOrder(
								centerline_order,
								geometry.centerlinePaths.length,
							)
						: geometry.centerlinePaths.map((_, index) => index);
					const nextReversed = keepsExistingAdjustments
						? geometry.centerlinePaths.map(
								(_, index) => Boolean(centerline_reversed[index]),
							)
						: geometry.centerlinePaths.map(() => false);

					setAttributes({
						outline_paths: geometry.outlinePaths,
						centerline_paths: geometry.centerlinePaths,
						path_lengths: geometry.pathLengths,
						centerline_order: nextOrder,
						centerline_reversed: nextReversed,
						logo_width: geometry.width,
						logo_height: geometry.height,
						use_dynamic_paths: true,
					});
				})
				.catch((error: unknown) => {
					console.error("Welcome path generation failed.", error);
				});
		}, 400);
	...
	useEffect(() => {
		...
		const animationTarget = hasDynamicPaths
			? blockRef.current.querySelector<SVGPathElement>(
					'[data-welcome-animation-last="true"]',
				)
			: blockRef.current.querySelector<SVGMaskElement>("#letters-svg-mask");
		if (!animationTarget) {
			return;
		}
		...
		const fallbackTimer = window.setTimeout(
			handleAnimationEnd,
			(hasDynamicPaths ? animation_duration : 5.5) * 1000 + 250,
		);
		return () => {
	...
	useEffect(() => {
		...
		if (!root || !hasDynamicPaths) {
			return;
		}

		let controls: ReturnType<typeof animateDynamicPen> | undefined;
		const animationFrame = window.requestAnimationFrame(() => {
			if (is_anime) {
				controls = animateDynamicPen(root, animation_duration);
			} else {
				positionDynamicPenAtStart(root);
			}
		});
	...
	return (
		<>
			<InspectorControls group="settings">
					...
					<PanelRow>
						<div style={{ width: "100%" }}>
							...
							<Select
								...
								value={fontFamilyOptions.find(
									(option) => option.value === welcome_font,
								)}
								onChange={(newOption: FontFamilyOption | null) => {
									setAttributes({
										welcome_font: newOption?.value ?? "",
										use_dynamic_paths: true,
									});
								}}
					...
					<PanelRow className="logoSizeCtrl">
						<RangeControl
							...
							onChange={(value: number) =>
								setAttributes({ logo_size: value, use_dynamic_paths: true })
							}
						/>
					</PanelRow>
					<PanelRow className="logoSizeCtrl">
						<RangeControl
							...
							withInputField={false}
							onChange={(value: number) =>
								setAttributes({ logo_gap: value, use_dynamic_paths: true })
							}
						/>
					</PanelRow>
					<PanelColorGradientSettings
				...
				<PanelBody
					title={__("Animation Settings", "opening-block")}
					initialOpen={false}
					className="animation_design_ctrl"
				>
					<PanelRow>
						<RangeControl
							...
							step={0.5}
							onChange={(value: number) =>
								setAttributes({ animation_duration: value })
							}
						/>
					</PanelRow>
					<PanelRow>
						<RangeControl
							value={skeleton_scale}
							label={__("Skeleton Simplification", "opening-block")}
							max={5}
							min={1}
							step={0.1}
							onChange={(value: number) =>
								setAttributes({
									skeleton_scale: value,
									use_dynamic_paths: true,
								})
							}
						/>
					...
					<PanelRow>
						<RangeControl
							value={mask_stroke_width}
							label={__("Mask Stroke Width", "opening-block")}
							max={40}
							min={1}
							step={1}
							onChange={(value: number) =>
								setAttributes({ mask_stroke_width: value })
							}
						/>
					</PanelRow>
				</PanelBody>
				...
				<PanelBody
					title={__("Centerline Preview", "opening-block")}
					initialOpen={false}
					className="centerline_preview_ctrl"
				>
					<div
						style={{
							display: "grid",
					...
					>
						{normalizedCenterlineOrder.map((sourceIndex) => {
							const isVisible =
								visibleCenterlineIndices.includes(sourceIndex);
							const isReversed = Boolean(
								centerline_reversed[sourceIndex],
							);
							return (
								<div
									key={`centerline-control-${sourceIndex}`}
									style={{
										display: "flex",
										alignItems: "center",
										...
										borderRadius: "2px",
										background:
											dragOverCenterlineIndex === sourceIndex
												? "#e0f0ff"
												: undefined,
									}}
									onDragOver={(event: DragEvent<HTMLDivElement>) =>
										handleCenterlineDragOver(event, sourceIndex)
									}
									onDrop={(event: DragEvent<HTMLDivElement>) =>
										handleCenterlineDrop(event, sourceIndex)
									}
								>
									<div
										style={{
									...
									>
										<span
											...
											role="button"
											tabIndex={0}
											aria-label={__("Drag to reorder", "opening-block")}
											style={{
												cursor: "grab",
												fontSize: "18px",
												lineHeight: 1,
											...
											onDragStart={(event: DragEvent<HTMLElement>) =>
												handleCenterlineDragStart(event, sourceIndex)
											}
											onDragEnd={handleCenterlineDragEnd}
										>
											⋮⋮
										</span>
										<span>
											{__("Centerline", "opening-block")} {sourceIndex + 1}
										</span>
									</div>
									<div
									...
									>
										<Button
											...
											onClick={() =>
												toggleCenterlineDirection(sourceIndex)
											}
										>
											{__("Reverse", "opening-block")}
										</Button>
										<Button
											...
											onClick={() =>
												toggleCenterlineVisibility(sourceIndex)
											}
										>
											{isVisible
												? __("Visible", "opening-block")
												: __("Hidden", "opening-block")}
										</Button>
									</div>
								</div>
							);
						...
						{centerline_paths.length === 0 && (
							<p>{__("No centerlines generated.", "opening-block")}</p>
						)}
					</div>
				</PanelBody>
			</InspectorControls>
					...
					<div id="splash_logo">
						<div className="wrapper wrapper__letters">
							<div className="letters is-active" data-wrapper-id="letters">
								<div
									className={`letters_line ${hasDynamicPaths ? "is-dynamic" : ""}`}
								>
									{hasDynamicPaths ? (
										<DynamicWelcomeSvg
											key={`dynamic-welcome-${isDynamicAnimationPlaying ? "playing" : "idle"}`}
											attributes={attributes}
											play={isDynamicAnimationPlaying}
											showCenterlines={!trigger_anime}
											visibleCenterlineIndices={visibleCenterlineIndices}
										/>
									) : (
								...
								<span
									className={`letters_pen ${hasDynamicPaths ? "is-dynamic" : ""} ${is_anime ? "play" : ""}`}
								></span>
							</div>
						</div>
					</div>
